...
func TestTriceVersion1(t *testing.T) {
	...
	var b bytes.Buffer
	e := args.Handler(&b, []string{"trice", "version"})
	assert.Nil(t, e)
...
func TestTriceWrong1(t *testing.T) {
	var b bytes.Buffer
	e := args.Handler(&b, []string{"trice", "wrong"})
...
func TestDisplayServerStartAndShutdown(t *testing.T) {
	...
	go args.Handler(&ds, []string{"trice", "ds", "-color", "none", "-ipp", "61496"})
	for ds.String() != "displayServer @ localhost:61496\n" {
	}
	args.Handler(&sd, []string{"trice", "sd", "-ipp", "61496"})
