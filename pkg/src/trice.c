...
int NextTrice( uint8_t** buf, size_t* len, uint8_t** pStart, size_t* pLen ){
    int triceType = *(uint16_t*)buf >> 14;
    int offset = 0;
    *pStart = *buf;
    switch( triceType ){
        case 0: // EX
...
void TriceOutMultiFastMode( uint32_t* tb, size_t tLen ){
    size_t len;
    size_t cLen;
