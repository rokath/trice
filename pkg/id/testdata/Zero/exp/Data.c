...
void TraceLogChecks( void )
{
    ...
    /* test normal traces */
    TL8_1( Id(0), "WRN: TL8-1 %d\n", -1 );
    TL8_2( Id(0), "ERR: TL8-2 %d +%d\n", -1, +2 );
    TL8_3( Id(0), "TIM: TL8-3 +%d %d +%d\n", +1, -2, +3 );
    TL8_4( Id(0), "RD_: TL8-4 %d +%d %d +%d\n", -1, +2, -3, +4 );
    TL8_5( Id(0), "WR_: TL8-5 +%d %d +%d %d +%d\n", +1, -2, +3, -4, +5 );
    TL8_6( Id(0), "ATT: TL8-6 %d %d +%d +%d %d %d \n", -1, -2, +3, +4, -5, -6 );
    TL8_7( Id(0), "DBG: TL8-7 +%d +%d %d %d +%d +%d %d\n", +1, +2, -3, -4, +5, +6, -7 );
    TL8_8( Id(0), "DIA: TL8-8 %d %d +%d +%d +%d +%d %d %d\n", -1, -2, +3, +4, +5, +6, -7, -8 );
    WaitForTraceLogTransmissionDone();

    TL16_1( Id(0), "ISR: TL16-1 %x\n", 0x1311 );
    TL16_1( Id(0), "SIG: TL16-1 %d\n", -1 );
    TL16_2( Id(0), "TST: TL16-2 +%d %d\n", +1, -2 );
    TL16_3( Id(0), "MSG: TL16-3 %d +%d %d\n", -1, +2, -3 );
    TL16_4( Id(0), "MSG: TL16-4 +%d %d %d +%d\n", +1, -2, -3, +4 );
    TL32_1( Id(0), "DBG: TL32-1 %x\r\n", 0x44332211 );
    TL32_2( Id(0), "DBG: TL32-2 %d +0x%x\r\n", -1, +2 ); // ok
    TL32_1( Id(0), "DBG: TL32-1 %x\r\n", 0x88332211 );
    TL32_2( Id(0), "att:TL32-2 %08x%08x\r\n", 0xfedcba98, 0x76543210 ); // ok
    TL32_1( Id(0), "DBG: TL32-1 %x\r\n", 0xaa332211 );
    TL8_1( Id(0), "SIG:BZA %X you can write a very long message here...\n", 0 );
    WaitForTraceLogTransmissionDone();
    #endif
...
void TraceLogTimeChecks( void )
{
    ...
    TL0( Id(0), "dbg:Shortest " );
    TL0( Id(0), "msg:Trace " ); // <-- this one is measured
    TL0( Id(0), "dbg:takes " );
    ...
    TL32_2( Id(0), "tim:%3u SysTicks =%2u us\r\n", Ticks, s );

    // longest trace
    WaitForTraceLogTransmissionDone();
    StartSysTickTimer( &Timer );
    TL0( Id(0), "dbg:Longest " );
    TL32_2( Id(0), "msg:Trace (%x %x) ", 0xffffeeee, 0xeeeeffff ); // <-- this one is measured
    TL0( Id(0), "dbg:takes " );
    Ticks3 = ElapsedSysTicks( &Timer );
...
void DelayChecks( void )
{
    ...
    s = TICKS_TO_MICROSECONDS( Ticks );
    TL32_2( Id(0), "tim:1000 SysTicks Delay duration: %u SysTicks = %u us\r\n", Ticks, s );

    WaitForTraceLogTransmissionDone();
    StartUsTimer( &Timer );
    DelayUs( 100u, NoopUsTicks ); // max 994 us with 1ms SysTick period
    s = ElapsedUs( &Timer );
    TL32_1( Id(0), "tim:100us Delay duration: = %u us\r\n", s );
    ...
    Ticks = ElapsedSysTicks( &Timer );
    s = TICKS_TO_MICROSECONDS( Ticks );
    TL32_2( Id(0), "tim:500us Delay duration: %u us = %u SysTicks\r\n", s, Ticks );
    ...
    DelayMs( 100, Pause );
    s = ElapsedMs( &Timer );
    TL32_1( Id(0), "tim:100ms Delay duration: %u ms\r\n", s );
    WaitForTraceLogTransmissionDone();
    #endif // #ifdef TRACELOG
}
...
void TraceRegisterSet( char* pName, uint32_t SetSize, uint32_t BaseAddress )
{
    ...
    TL32_2( Id(0), "msg:    // BaseAddress=0x%08x, RegCount=%2u, ", BaseAddress, RegCount );
    TraceString( pName );
    TL0( Id(0), "\r\n" );
    for( i = 0; i < RegCount; i++ )
    {
        ...
        if( RegisterValue )
        {
            TL32_2( Id(0), "wr_:    {  (uint32_t*) 0x%08x, 0x%08xu  },", RegisterAddress, RegisterValue );
            TL8_2(  Id(0), "msg: // %3u = 0x%02x  ", n, n );
            n++;
            TraceString( pName );
            TL0( Id(0), "\r\n" );
            WaitForTraceLogTransmissionDone();
...
void DumpMemory( uint32_t Address, uint32_t Size )
{
    int i;
    int Count = Size>>2;
    TL32_2( Id(0), "att:    // Memory Dump: Address=0x%08x, Count=%2u\r\n", Address, Count );
    for( i = 0; i < Count; i++ )
    {
        ...
        if( Value )
        {
            TL32_2( Id(0), "rd_:    {  (uint32_t*) 0x%08x, 0x%08xu  },\r\n", pAddress, Value );
            WaitForTraceLogTransmissionDone();
        }
    }
}
