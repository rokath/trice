...
Yes, you can simply start `trice ds` inside a console (option: `third_party/alacritty`) locally or on a remote PC and connect with several trice tool instances with `trice log -p COM15 -ds`.
...
- For compiler adaption see `trice/pkg/src/intern/triceConfigCompiler.h`.
- For hardware adaption see `trice/pkg/src/intern/triceUART_LL_STM32.h`
