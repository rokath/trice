...
static void triceNonBlockingWrite( int ticeID, uint8_t* pBuf, size_t len );
...
static int TriceSingleEncode(uint32_t* addr, int* pTriceID, uint8_t** pBuf, size_t* pLen);
...
static void triceNonBlockingWrite( int triceID, uint8_t* pBuf, size_t len ){
...
//! TriceSingleDeferredOut expects a single trice at addr and returns the wordCount amount of this trice which includes 1-3 padding bytes.
