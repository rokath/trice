...
void nodeSendDivideResult(node_t* node, float value) {
	...
	// Demo-only TX trace: the Trice record itself is still sent unchanged.
	nodePrintLineF("%s: tx:abc:DivideResult value=%f\n", node->name, (double)value);
}
