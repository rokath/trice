...
const char* bcSimErrorString(int err) {
	...
	case BCSIM_ERR_LOCK:
		return "lock error";
	case BCSIM_ERR_LOCK_TIMEOUT:
		return "lock timeout";
	case BCSIM_ERR_RANGE_OVERFLOW:
		return "own-range table overflow";
	case BCSIM_ERR_PATH_TOO_LONG:
		return "path or name too long";
	case BCSIM_ERR_NOT_OPEN:
