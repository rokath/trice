...
case "$OSTYPE" in
  darwin*)  make -j $(sysctl -n hw.ncpu) TRICE_FLAGS="-D$ARG1 -D$ARG2" gcc   -f OS_Darwin.mak ;; 
  linux*)   make -j $(nproc --all)       TRICE_FLAGS="-D$ARG1 -D$ARG2" gcc   -f OS_Linux.mak ;;
  msys*)    make -j $(nproc --all)       TRICE_FLAGS="-D$ARG1 -D$ARG2" gcc   -f OS_Windows.mak ;;
  cygwin*)  make -j $(nproc --all)       TRICE_FLAGS="-D$ARG1 -D$ARG2" clang -f OS_Windows.mak ;;
  *)        echo "unknown: $OSTYPE" ;;
