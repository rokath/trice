...
ARG1=${1:-NoArgs} # https://stackoverflow.com/questions/9332802/how-to-write-a-bash-script-that-takes-optional-input-arguments

if [[ $ARG1 == NoArgs ]]; then
   ARGS=
elif [[ $ARG1 == TRICE_OFF=0 ]]; then
   ARGS=
elif [[ $ARG1 == TRICE_OFF=1 ]]; then
   ARGS="TRICE_FLAGS=-DTRICE_OFF=1"
else
   echo arg1="$ARG1" - Use only TRICE_OFF=1 or TRICE_OFF=0 or nothing as argument.
   exit
fi
...
#make -j $(nproc --all) $ARGS # Windows
make -j $(sysctl -n hw.ncpu) $ARGS # MacOS
