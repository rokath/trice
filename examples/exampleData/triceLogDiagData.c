...
void TriceLogDiagnosticData(void) {
	...
	if ((RTT0_wrSpaceMin < TRICE_BUFFER_SIZE)) {
		trice16(iD(16383), "WARNING:RTT0_writeDepthMax=%u (BUFFER_SIZE_UP=%u)\n", RTT0_writeDepthMax, BUFFER_SIZE_UP);
	} else {
		trice16(iD(16382), "diag:RTT0_writeDepthMax=%u (BUFFER_SIZE_UP=%u)\n", RTT0_writeDepthMax, BUFFER_SIZE_UP);
	}
	...
	if (TriceDynStringBufTruncateCount != 0) {
		trice16(iD(15556), "wrn:TriceDynStringBufTruncateCount = %u\n", TriceDynStringBufTruncateCount);
	}
