...
void TriceLogDiagnosticValues( void ){
    ...
    if( triceSingleDepthMax <= TRICE_BUFFER_SIZE ){
        TRice16( iD(16381), "diag:triceSingleDepthMax =%4u of %d (TRICE_BUFFER_SIZE)\n", triceSingleDepthMax, TRICE_BUFFER_SIZE );
    }else{
        TRice16( iD(16380), "err:triceSingleDepthMax =%4u of %d (TRICE_BUFFER_SIZE overflow!)\n", triceSingleDepthMax, TRICE_BUFFER_SIZE );
    }
    
    if( TriceDataOffsetDepthMax < 0 || TriceDataOffsetDepthMax >= TRICE_DATA_OFFSET ){
        TRice16( iD(16379), "err:TriceDataOffsetDepthMax = %d of %d (TRICE_DATA_OFFSET\n", TriceDataOffsetDepthMax, TRICE_DATA_OFFSET );
    }else if( TriceDataOffsetDepthMax != 0 ){
        TRice16( iD(16378), "diag:TriceDataOffsetDepthMax = %d of %d (TRICE_DATA_OFFSET\n", TriceDataOffsetDepthMax, TRICE_DATA_OFFSET );
    }

    if(TriceDynBufTruncateCount != 0){
        trice( iD(16377), "err: TriceDynBufTruncateCount = %u\n", TriceDynBufTruncateCount );
    }
    
#if TRICE_PROTECT == 1

    if(TriceDirectOverflowCount != 0){
        trice( iD(16376), "err: TriceDirectOverflowCount = %u\n", TriceDirectOverflowCount );
    }
    if(TriceDeferredOverflowCount != 0){
        trice( iD(16375), "err: TriceDeferredOverflowCount = %u\n", TriceDeferredOverflowCount );
    }

#endif

    if(TriceErrorCount > 0){
        trice( iD(16374), "err: TriceErrorCount = %u\n", TriceErrorCount );
    }
