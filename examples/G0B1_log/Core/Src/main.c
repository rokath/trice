...
#include "trice.h"
#include "trice_main_c_KB93C5977DE12EC66.h" // trice-bind: keep as last include before this file's Trice calls
...
// TriceLocalUartWrite is intentionally blocking: only the low-priority logging
// task calls it, while producer tasks and interrupts merely enqueue Trices.
static void TriceLocalUartWrite(const char* text, size_t length) {
...
void StartTask02(void const* argument) {
	...
	TRICE_UNUSED(argument)
	TRice("msg:StartTask02:TriceLog to USART2\n");
	/* Infinite loop */
	for (;;) {
#if !TRICE_OFF
		char text[160];
		...
		// A negative result consumed the offending complete record. The task
		// deliberately continues so later records can still be reported.
		TRICE_UNUSED(length)
#endif // #if !TRICE_OFF
		osDelay(10);
	}
