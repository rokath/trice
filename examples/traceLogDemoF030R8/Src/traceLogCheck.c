...
\details an subtrace is one tracelog, if param size max 2 bytes. traces with more bytes as parameter need several subtraces (max 4).
...
void TraceLogCheckSet( void )
{
    ...
    TL0 (Id(43853), "--------------------------------------------------------------------------------------------------------------------------------------------------\n" );
    // 4 subtraces
} // sum 111 sub traces (needs 444 bytes)
