...
type bindDiscoveredJSON struct {
	...
	til      TriceIDLookUp
	li       TriceIDLookUpLI
	byFormat bindFormatIndex
}

// bindFormatIndex is the immutable reverse view of one discovered TIL. It
// prevents every Trice site from linearly scanning the same complete catalog.
type bindFormatIndex map[TriceFmt][]TriceID

// bindLocationMatchKey identifies one path comparison whose result is stable
// for the duration of a bind command.
type bindLocationMatchKey struct {
	stored       string
	metadataPath string
	sourcePath   string
}

// bindLocationMatchCache avoids repeating filepath normalization and relative
// path construction for every ID that shares the same LI source file.
type bindLocationMatchCache map[bindLocationMatchKey]bool
...
// sidecars while each source receives its own proximity order. All cached data
// remains read-only.
type bindMetadataResolver struct {
	...
	sidecars    map[string]*bindHistoricalSidecar
...
func newBindMetadataResolver(w io.Writer, fSys *afero.Afero) *bindMetadataResolver {
	...
	return &bindMetadataResolver{
		...
		sidecars:    make(map[string]*bindHistoricalSidecar),
...
func (resolver *bindMetadataResolver) readMetadataJSON(path string) *bindDiscoveredJSON {
	...
	metadata := &bindDiscoveredJSON{path: path, dir: filepath.Dir(path), kind: kind, til: til, li: li}
	if kind == bindMetadataTIL {
		metadata.byFormat = newBindFormatIndex(til)
	}
	return metadata
}
...
func (resolver *bindMetadataResolver) sidecarsForPlan(plan *bindFilePlan, directories []string) []bindHistoricalSidecar {
	...
	for rank, directory := range candidates {
		...
		seen[key] = true
		if sidecar, ok := resolver.sidecarForPlanAt(plan, directory.path, rank, directory.primary); ok {
...
// primarySidecarsForPlan reads only the authoritative output-tree sidecar.
// A complete match there outranks every implicit historical source, allowing
// the common no-change bind run to avoid discovering unrelated JSON catalogs.
func (resolver *bindMetadataResolver) primarySidecarsForPlan(plan *bindFilePlan) []bindHistoricalSidecar {
	sidecar, ok := resolver.sidecarForPlanAt(plan, BindDir, 0, true)
	if !ok {
		return nil
	}
	return []bindHistoricalSidecar{sidecar}
}

// sidecarForPlanAt validates and parses one exact owner sidecar without ever
// copying its generated bytes into another output directory.
func (resolver *bindMetadataResolver) sidecarForPlanAt(plan *bindFilePlan, directory string, rank int, primary bool) (bindHistoricalSidecar, bool) {
	if plan.key == "" || plan.sidecarName == "" {
		return bindHistoricalSidecar{}, false
	}
	path := filepath.Join(directory, plan.sidecarName)
	cacheKey := bindComparablePath(path)
	if cached, exists := resolver.sidecars[cacheKey]; exists {
		if cached == nil {
			return bindHistoricalSidecar{}, false
		}
		sidecar := *cached
		sidecar.rank = rank
		sidecar.primary = primary
		return sidecar, true
	}
	content, err := resolver.fSys.ReadFile(path)
	if errors.Is(err, os.ErrNotExist) {
		resolver.sidecars[cacheKey] = nil
		return bindHistoricalSidecar{}, false
	}
	if err != nil {
	...
	resolver.sidecars[cacheKey] = &bindHistoricalSidecar{path: path, sites: sites}
	return bindHistoricalSidecar{path: path, rank: rank, primary: primary, sites: sites}, true
}
...
func preferredBindIDs(w io.Writer, plans []bindFilePlan, resolver *bindMetadataResolver) map[bindSiteReference]TriceID {
	...
	locationMatches := make(bindLocationMatchCache)
	primaryByFormat := newBindFormatIndex(IDData.idToTrice)
	...
	for planIndex := range plans {
		...
		unresolved := make([]int, 0, len(plan.sites))
		for siteIndex := range plan.sites {
			if !plan.sites[siteIndex].wasExplicit {
				unresolved = append(unresolved, siteIndex)
			}
		}

		// The current output-tree sidecar has absolute candidate precedence. If
		// it resolves every live site, secondary JSON discovery cannot alter the
		// result and would only slow the normal unchanged bind run. Verbose mode
		// still performs complete discovery so it can report optional evidence.
		if !Verbose && len(unresolved) > 0 {
			primaryMetadata := bindPlanMetadata{sidecars: resolver.primarySidecarsForPlan(plan)}
			primaryCandidates := bindSidecarCandidates(plan, primaryMetadata, IDData.idToTrice)
			...
			for _, siteIndex := range unresolved {
				...
				candidates := compactBindIDCandidates(append([]bindIDCandidate(nil), primaryCandidates[siteIndex]...))
				if !acceptBindPreferredID(w, resolver, plan, site, ref, bindSiteFormat(site), candidates, claimed, primaryByFormat, locationMatches, preferred) {
					remaining = append(remaining, siteIndex)
				}
			}
...
// acceptBindPreferredID applies the established collision, location, and
// ownership checks to an already ordered candidate list. It is shared by the
...
func acceptBindPreferredID(w io.Writer, resolver *bindMetadataResolver, plan *bindFilePlan, site bindSite, ref bindSiteReference, format TriceFmt, candidates []bindIDCandidate, claimed map[TriceID]bindSiteReference, primaryByFormat bindFormatIndex, locationMatches bindLocationMatchCache, preferred map[bindSiteReference]TriceID) bool {
	for _, candidate := range candidates {
		...
		if primaryExists {
			...
			if location, hasLocation := IDData.idToLocRef[candidate.id]; hasLocation && !locationMatches.matches(location.File, LIFnJSON, plan.path) {
				resolver.verboseOnce(
		...
		if !primaryExists {
			...
			primaryByFormat.add(format, candidate.id)
		}
...
func bindCatalogCandidates(plan *bindFilePlan, site bindSite, format TriceFmt, metadata bindPlanMetadata, primaryByFormat bindFormatIndex, locationMatches bindLocationMatchCache) []bindIDCandidate {
	...
	for _, ranked := range metadata.json {
		...
		for _, id := range ranked.data.byFormat[format] {
			location, hasLocation, matches := bindSecondaryLocation(id, ranked.data.dir, plan.path, metadata, locationMatches)
			if hasLocation && !matches {
				continue
			}
...
// source lookup remains authoritative and is never mutated through this index.
func newBindFormatIndex(lookup TriceIDLookUp) bindFormatIndex {
	index := make(bindFormatIndex)
	for id, format := range lookup {
		index[format] = append(index[format], id)
	}
	for format := range index {
		sort.Slice(index[format], func(i, j int) bool { return index[format][i] < index[format][j] })
	}
	return index
}

// add records one selected secondary mapping in the live primary index. This
// retains the previous behavior in which later sites see earlier imports.
func (index bindFormatIndex) add(format TriceFmt, id TriceID) {
	ids := index[format]
	position := sort.Search(len(ids), func(i int) bool { return ids[i] >= id })
...
func bindSecondaryLocation(id TriceID, tilDirectory, sourcePath string, metadata bindPlanMetadata, locationMatches bindLocationMatchCache) (TriceLI, bool, bool) {
	var first TriceLI
	found := false
	for _, ranked := range metadata.json {
		...
		if locationMatches.matches(location.File, ranked.data.path, sourcePath) {
...
// matches memoizes the established location comparison without changing its
// path or suffix semantics.
func (cache bindLocationMatchCache) matches(stored, metadataPath, sourcePath string) bool {
	key := bindLocationMatchKey{stored: stored, metadataPath: metadataPath, sourcePath: sourcePath}
	if matches, ok := cache[key]; ok {
		return matches
	}
	matches := bindLocationMatches(stored, metadataPath, sourcePath)
	cache[key] = matches
	return matches
}
...
// only. All parser offsets refer to the unchanged source, so one final linear
// transformation replaces the former full-file copy for every individual ID.
func applyBindPreferredIDs(source []byte, planIndex int, plan *bindFilePlan, preferred map[bindSiteReference]TriceID) ([]byte, error) {
	...
	for siteIndex := range plan.sites {
		...
		edits = append(edits, idSourceEdit(original, 0, location[:], bindSiteFormat(plan.sites[siteIndex]), id))
	}
	if len(edits) == 0 {
		return source, nil
	}
	result, err := applySourceEdits(original, edits)
	if err != nil {
		return nil, err
	}
