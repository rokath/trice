...
`
	...
	assert.Contains(t, sourceText, "static void triceAbcCall_get_power_state(const triceAbcRx_t* rx)")
	assert.Contains(t, sourceText, "get_power_state(rx);")
	assert.Contains(t, sourceText, "static void triceAbcCall_set_time(const triceAbcRx_t* rx)")
	assert.Contains(t, sourceText, "set_time(rx);")
	assert.Contains(t, sourceText, "{ 1001u, 0u, triceAbcCall_get_power_state }")
	assert.Contains(t, sourceText, "{ 1003u, 32u, triceAbcCall_set_time }")
