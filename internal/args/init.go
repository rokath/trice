...
`) // flag
	...
	fsScLog.StringVar(&decoder.ShowTargetLocation, "tLocFmt", "%12s:%4d ", `Target location format string at start of each line, if target location existent (configured). Use "" to suppress existing target location. If several trices form a log line only the location of first trice ist displayed.`)
