...
"LOCmicro" means local time with microseconds. "UTCmicro" shows timestamps in universal time. When set to "off" no PC timestamps displayed.`) // flag
	...
	fsScLog.StringVar(&decoder.TypeX0, "typeX0", "error", `Selector-0/typeX0 handling for TREX logs. "error" reports X0 packets and is the default. "ignore" and "counted:ignore" silently discard valid counted X0 packets but still report malformed ones. "all:ignore" discards the complete selector-0 package without length checks and must only be used for non-mixed X0 packages. "counted:<format>" prints the counted payload with Go fmt. A value without ':' is shorthand for "counted:<format>".`)
	fsScLog.StringVar(&decoder.LocationInformationFormatString, "liFmt", decoder.LiFmtDefault, `Target location format string at start of each line, if target location existent (configured). Use "off" or "none" to suppress existing target location. If several trices form a log line only the location of first trice ist displayed.`)
	...
	fsScLog.StringVar(&decoder.TargetStamp32Delta, "ts32delta", "", `32-bit Target stamp delta format string at start of each line, if 32-bit target stamps existent (configured). Shows the delta to the previous displayed 32-bit target stamp with wraparound support. On the first value, trice prints an aligned placeholder ("-" for simple numeric formats, blanks for built-in "µs"/"us"/"ms"). Use "" to suppress this delta column completely for 32-bit stamp lines. Use "µs" (or "us"), "ms", or a format string like "...%d...". "epoch..." is not supported for deltas.`)
	fsScLog.StringVar(&decoder.TargetStamp16Delta, "ts16delta", "", `16-bit Target stamp delta format string at start of each line, if 16-bit target stamps existent (configured). Shows the delta to the previous displayed 16-bit target stamp with wraparound support. On the first value, trice prints an aligned placeholder ("-" for simple numeric formats, blanks for built-in "µs"/"us"/"ms"). Use "" to suppress this delta column completely for 16-bit stamp lines. Use "µs" (or "us"), "ms", or a format string like "...%d...".`)
