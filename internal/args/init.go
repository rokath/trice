...
`) // flag
	...
	fsScLog.StringVar(&decoder.ShowTargetStamp32, "tsf32", decoder.DefaultStamp32, `Target stamp format string at start of each line, if 32-bit target stamps existent (configured). Use "" to suppress existing target timestamps. If several trices form a log line only the timestamp of first trice ist displayed. The default is 3 fields with underscore.`)
	fsScLog.StringVar(&decoder.ShowTargetStamp16, "tsf16", decoder.DefaultStamp16, `Target stamp format string at start of each line, if 16-bit target stamps existent (configured). Use "" to suppress existing target timestamps. If several trices form a log line only the timestamp of first trice ist displayed. The default is us format with underscore.`)
	fsScLog.StringVar(&decoder.ShowTargetStamp0, "tsf0", "tim:             ", `Target stamp format string at start of each line, if no target stamps existent (configured). Use "" to suppress existing target timestamps. If several trices form a log line only the timestamp of first trice ist displayed.`)
