...
func logInit() {
	...
	fsScLog.StringVar(&translator.Encoding, "encoding", "COBS", `The trice transmit data format type, options: '(CHAR|COBS|dumpDec|ESC|FLEX)'. Target device encoding must match. 
...
`) // flag
	fsScLog.StringVar(&translator.Encoding, "e", "COBS", "Short for -encoding.") // short flag
