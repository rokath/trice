...
"LOCmicro" means local time with microseconds. "UTCmicro" shows timestamps in universal time. When set to "off" no PC timestamps displayed.`) // flag
	...
	fsScLog.StringVar(&decoder.ShowTargetStamp32, "tsf32", decoder.DefaultStamp32, `Target stamp format string at start of each line, if 32-bit target stamps existent (configured). Options: "hh:mm:ss_ms" or s.th. like "...%d...".  Use "" to suppress existing target timestamps. If several trices form a log line only the timestamp of first trice ist displayed.`)
	fsScLog.StringVar(&decoder.ShowTargetStamp16, "tsf16", decoder.DefaultStamp16, `Target stamp format string at start of each line, if 16-bit target stamps existent (configured). Options: "s_ms" or s.th. like "...%d...". Use "" to suppress existing target timestamps. If several trices form a log line only the timestamp of first trice ist displayed. The default is us format with underscore.`)
...
Example: "-pick err:wrn -pick default" results in suppressing all messages despite of as error, warning and default tagged messages. Not usable in conjunction with "-ban".`) // multi flag
	fsScLog.StringVar(&decoder.PackageFraming, "packageFraming", "TCOBSv1", `Use "COBS" as alternative. "COBS" needs "#define TRICE_FRAMING TRICE_FRAMING_COBS" inside "triceConfig.h".`)
