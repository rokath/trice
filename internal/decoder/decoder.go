...
// OutputKind identifies whether decoded text is application output or a tool
// diagnostic. Decoders without OutputClassifier retain the legacy behavior and
// return application output only.
type OutputKind uint8

const (
	// OutputApplication marks text originating from a valid target log event.
	OutputApplication OutputKind = iota

	// OutputDiagnostic marks decoder or transport diagnostics generated by the tool.
	OutputDiagnostic
)

// OutputSpan classifies one contiguous range in the buffer returned by Read.
...
// diagnostics from application output before application filters are applied.
// At most one range may be application output. The returned slice is valid
// until the next Read and must not be modified.
type OutputClassifier interface {
	DecodedOutputSpans() []OutputSpan
}
