...
func TestLogs(t *testing.T) {
	...
	triceLog1 := func(t *testing.T, fSys *afero.Afero, buffer string) string {
		...
		assert.Nil(t, args.Handler(io.Writer(&o), fSys, []string{"trice", "log", "-i", path.Join(triceDir, "/demoTIL.json"), "-p=BUFFER", "-args", buffer, "-hs=off", "-prefix=off", "-li=off", "-color=off", "-pf=COBS", "-d16=false", "-ts16", "time:    %04x", "-ts32", "time:%08x"}))
		return o.String()
	}
