...
func TestLogs(t *testing.T) {
	...
	triceLog := func(t *testing.T, fSys *afero.Afero, buffer string) string {
		var o bytes.Buffer
		assert.Nil(t, args.Handler(io.Writer(&o), fSys, []string{"trice", "log", "-singleFraming", "-i", path.Join(triceDir, "/demoTIL.json"), "-p=BUFFER", "-args", buffer, "-triceEndianness=bigEndian", "-hs=off", "-prefix=off", "-li=off", "-color=off", "-ts0", "time:        ", "-ts16", "time:    %04x", "-ts32", "time:%08x"}))
		return o.String()
	}
