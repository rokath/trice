Bitte schaue dir den Promt an in ./docs/trice_logging_handover/Command.md im Hinblick auf Klarheit und stelle Rückfragen dazu. Noch nichts machen. Erstmal nur helfen die Aufgabe zu präzisieren.
...
- noch offen und ohne Targetcode Relevanz: 
  ...
  - Dass -ulabel tagA:tagb falsch implementiert ist (L06) ist noch nicht aufgefallen, also hat es niemand verwendet. Damit sollte am einfachsten die Dokumentation -ulabel tagA:tagB verbieten. Hier bitte den Go-Code (Help-Text) anpassen.
...
- (+)Tag-Routing im Taget: (implementiert über ID Control (Defines) und ID Routing) Verhalten wird zur Compile-Time definiert, aber das Routing erfolgt zur Laufzeit anhand der IDs. Vielleicht muss hier die Doku verbessert werden.

Da -logLevel mehr Probleme macht als Nutzen, würde ich in der Doku von -logLevel Benutzung abraten, allein, weil es z.B. die andern Komponenten einer LogLine, wie Stamps, beeinflusst. Für User die gerne -logLevel verwenden wollen, sollte eine komplette Beispielkonfiguration angegeben werden, die insbesondere die einzelnen Logline Teile berücksichtigt. Test dazu wäre toll.
...
- Für bind verbindlich ist til.json. Was aber, wenn eine Trice Message mehrfach da ist und li.json und Sidecar nicht übereinstimmen? Beides sind quasi Build-Artefakte, die aber genau in solchem Fall zu Rate gezogen werden. Wer soll gewinnen? Das Änderungsdatum entscheiden lassen?
- Fehler durch Tag-Doppelung einfach jetzt benennen, damit ich sie auf dem kurzen Weg kann. Es sollte dafür ein Test entstehen (Issue anlegen)
- -pick/-ban mit unbekanntem Tag -> CLI Fehler (Abbruch), allerdings -ulabel tagX erlaubt -pick/-ban tagX. Test erforderlich.
- logLevel Filterung sollte für LogLine Teile wie stamps nicht greifen, wenn der Implemetnierungsaufand nicht zu hoch ist. Tests sind nötig dafür.
...
- L03: ID-Policy Änderungen -> Das soll nur auf neu zu vergebende IDs wirken. Keine übermäßige Meldungsvielfalt. Es könnte vielleicht einfach schweigend operiert werden und nur wenn -v (Verbose) benutzt wird, kommt eine Warnung, welche bereits vergebenen IDs in til.json der neuen Policy widersprechen.
...
- L06: -ulabel colon List -> einfach in Doku verbieten. Das erlaubt ggf. eine optionale -ulabel Erweiterung mit einem Listengewicht, falls mal gefordert. -ulabel sollte aktuell colons als CLI Fehler behandeln.
...
- L27: IDRange Fehler wie Überlappung sind harte CLI-Fehler. Tests!
- Überarbeitetes Handover komplett auf Deutsch daneben
- Insgesamt sind es also 4 Aufgabengruppen: Die ooben genannten drei plus die Issue-Kollektion, wobei es aktuell noch Überlappungen gibt.
...
- Handover2_DE aus Handover_DE ableiten und darin
  ...
  - Offene Fragen als Liste. Darin zuerst Lese-Hinweise für das Handover2_DE, welches auch die einzelnen Dateien und ihren Sinn beschreibt. Nichts wiederholen. Einfach einen Abarbeitungsfaden bereitsdtellen.
