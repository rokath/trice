...
There was a learning **not** to reduce the transmit byte count to an absolute minimum, but to focus more on `TRICE` macro speed and universality. That led to a double buffer on the target side discarding the previous FIFO solution and the [COBS package descriptor](./TriceMessagesEncoding.md#2-cobs-encoding-and-user-protocols) allowing alongside user protocols.
