#!/usr/bin/env bash
#
# Test 510: Exercises public Bind workflows and transactional state restoration.
...
test_public_workflows() {
  ...
  run_silent_workflow "$SCRIPT_DIR/_230_legacy_insert_ids.sh" || return 1
  ...
  run_silent_workflow "$SCRIPT_DIR/_240_legacy_clean_ids.sh" || return 1
  ...
  run_silent_workflow "$SCRIPT_DIR/_250_legacy_remigrate_bind_to_clean.sh" || return 1
...
# by _140_trice_test_state.sh and therefore exercises production wrapper behavior.
state_child() {
  state_worker_mode="$1"
...
run_state_case() {
  ...
  "$SCRIPT_DIR/_510_test_bind_workflows.sh" --quiet --state-child "$mode" || actual=$?
