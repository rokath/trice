...
# Refresh the canonical ID data and generated PC-test harness files.

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd -- "$SCRIPT_DIR/.." && pwd)"
cd "$REPO_ROOT" || exit 1
source "$SCRIPT_DIR/_120_setup_trice_environment.sh"
TD="./_test/testdata"
...
#    ls -l _test | grep -v testdata | grep -v -i ReadMe | cut -c 40- >> ./scripts/_330_renew_ids_and_refresh_tests.sh
#    "/c/Program Files/Notepad++/notepad++.exe" ../scripts/_330_renew_ids_and_refresh_tests.sh
