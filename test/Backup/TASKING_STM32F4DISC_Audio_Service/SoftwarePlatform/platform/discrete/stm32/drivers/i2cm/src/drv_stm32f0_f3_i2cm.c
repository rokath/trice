...
int i2cm_putchar( i2cm_t * restrict drv, bool start, const uint8_t data )
{
    if ( start )
    {
        ...
        while (I2C_ReadRegister(drv->i2cx,  I2C_Register_CR2) & I2C_CR2_START) wait();
    }
...
int i2cm_getchar( i2cm_t * restrict drv, bool start, bool ack )
{
    ...
    {
        I2C_GenerateSTART( drv->i2cx, ENABLE );
        while (I2C_ReadRegister(drv->i2cx,  I2C_Register_CR2) & I2C_CR2_START);
    }
    ...
    // Start receiver
    while (!I2C_GetFlagStatus( drv->i2cx, I2C_FLAG_RXNE))
    {
...
int i2cm_read_reg( i2cm_t * restrict drv, uint8_t slave, uint32_t reg, size_t addrsize, void * data, size_t length )
{
    ...
    if ( addrsize )
    {
        I2C_NumberOfBytesConfig( i2c, (uint8_t)addrsize );
        I2C_SlaveAddressConfig( i2c, slave );
        I2C_MasterRequestConfig( i2c, I2C_Direction_Transmitter );
        I2C_GenerateSTART( i2c, ENABLE );
        timeout = clock() + I2CM_TIMEOUT;
        while ( I2C_ReadRegister( i2c,  I2C_Register_CR2 ) & I2C_CR2_START )
        {
            if ( clock() > timeout ) return i2cm_abort( drv, -1 );
        }
    ...
    I2C_NumberOfBytesConfig( i2c, (uint8_t)addrsize + (uint8_t)length );
    I2C_SlaveAddressConfig( i2c, slave );
    ...
    timeout = clock() + I2CM_TIMEOUT;
    while (I2C_ReadRegister(drv->i2cx,  I2C_Register_CR2) & I2C_CR2_STOP)
    {
