...
#include <per_stm32f37x_i2s_cfg_instance.h>
...
__INTERRUPT_NATIVE void stm32f37x_i2sm_handler_native( void )
{
    ...
    case DMA1_Channel5_IRQn:
        ...
        break;
    case DMA2_Channel1_IRQn:
        DMA_ClearFlag( DMA2_FLAG_TC1 );
        break;
    case DMA2_Channel2_IRQn:
...
i2sm_t * i2sm_open( unsigned int id )
{
    ...
    const per_stm32f37x_i2s_cfg_instance_t * restrict per_cfg;
    drv_cfg = &drv_stm32_i2sm_instance_table[id];
    per_cfg = &per_stm32f37x_i2s_instance_table[drv_cfg->per_stm32_i2s];
    ...
    if (drv->dma_stream == DMA1_Channel2)
    { irq = DMA1_Channel2_IRQn; }
    else if (drv->dma_stream == DMA1_Channel3)
    { irq = DMA1_Channel3_IRQn; }
    else if (drv->dma_stream == DMA1_Channel4)
    ...
    interrupt_register_native(irq, (void*) drv, stm32f37x_i2sm_handler_native);
