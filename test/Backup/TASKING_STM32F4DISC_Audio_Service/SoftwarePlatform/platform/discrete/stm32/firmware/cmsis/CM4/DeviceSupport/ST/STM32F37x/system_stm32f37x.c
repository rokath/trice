...
#if STM32F37X_CMSIS_CLKSRC == STM32F37X_CMSIS_CLKSRC_HSI
#endif
#if STM32F37X_CMSIS_CLKSRC == STM32F37X_CMSIS_CLKSRC_HSE
#  define SYSCLOCK_SETUP_HSE
#  define SYSCLOCK_CLKSRC  RCC_CFGR_SW_HSE
#endif
#if STM32F37X_CMSIS_CLKSRC == STM32F37X_CMSIS_CLKSRC_PLLCLKBYHSI
#  define SYSCLOCK_SETUP_PLL
#  define SYSCLOCK_PLL_INPUT  RCC_CFGR_PLLSRC_HSI_Div2
#  define SYSCLOCK_CLKSRC  RCC_CFGR_SW_PLL
#endif
#if STM32F37X_CMSIS_CLKSRC == STM32F37X_CMSIS_CLKSRC_PLLCLKBYHSE
#  define SYSCLOCK_SETUP_HSE
#  define SYSCLOCK_SETUP_PLL
...
#if STM32F37X_CMSIS_CLOCKHZ <= 24000000
#define SYSCLOCK_FLASHLATENCY 0
#elif STM32F37X_CMSIS_CLOCKHZ <= 48000000
...
/** @addtogroup STM32F37x_System_Private_Variables
  * @{
  */
uint32_t SystemCoreClock    = STM32F37X_CMSIS_CLOCKHZ;
...
static void SetSysClock(void)
{
  ...
  RCC->CFGR |= (STM32F37X_CMSIS_MCOSRC << 24);

  /* At this stage the HSI is enabled and used as System clock source */

#ifdef SYSCLOCK_SETUP_HSE
// use pincfg document setting or backup from core plugin
# if defined(PINCFG_RCC_OSC_HSE_EXTERNAL_CLOCK_SOURCE) || (defined(STM32F37X_CMSIS_HSE_BYPASS) && STM32F37X_CMSIS_HSE_BYPASS)
...
# endif
  ...
  {
    /* HCLK */
    RCC->CFGR |= (STM32F37X_CMSIS_AHB << 4);

    /* PCLK2 */
    RCC->CFGR |= (STM32F37X_CMSIS_APB2 << 11);

    /* PCLK1 */
    RCC->CFGR |= (STM32F37X_CMSIS_APB1 << 8);

#ifdef SYSCLOCK_SETUP_PLL
    /* PLL configuration */
    RCC->CFGR |= (SYSCLOCK_PLL_INPUT | ((STM32F37X_CMSIS_PLL_MUL - 2) << 18));
# ifdef SYSCLOCK_SETUP_HSE
    RCC->CFGR2 |= (STM32F37X_CMSIS_HSE_PREDIV - 1);
# endif
...
#ifdef STM32F37X_CMSIS_USB_DIV
  /* Configure USB clock */
  RCC->CFGR |= (STM32F37X_CMSIS_USB_DIV << 22);
#endif
}
