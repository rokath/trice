# 1 "../Core/Src/stm32f0xx_it.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 365 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Core/Src/stm32f0xx_it.c" 2
# 22 "../Core/Src/stm32f0xx_it.c"
# 1 "../Core/Inc\\main.h" 1
# 31 "../Core/Inc\\main.h"
...
# 23 "../Core/Src/stm32f0xx_it.c" 2
# 1 "../Core/Inc\\stm32f0xx_it.h" 1
# 50 "../Core/Inc\\stm32f0xx_it.h"
...
void USART2_IRQHandler(void)
{
    ...
    if (LL_USART_IsActiveFlag_RXNE(((USART_TypeDef *) (0x40000000UL + 0x00004400UL))) ) {
        static char rxBuf[120 +1];
        static int index = 0;
        uint8_t v;
        if( LL_USART_IsActiveFlag_ORE(((USART_TypeDef *) (0x40000000UL + 0x00004400UL))) ){
            { uint32_t primaskstate = __get_PRIMASK(); __disable_irq(); { { uint32_t ts = TriceStamp32(); do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = ((0xC000|(6470))); TriceBufferWritePosition = (uint32_t*)p; }while(0); do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (ts); TriceBufferWritePosition = (uint32_t*)p; }while(0); do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (((ts) >> 16)); TriceBufferWritePosition = (uint32_t*)p; }while(0);; }; do{ uint16_t v = ((0)<<8) | TriceCycle++; do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (v); TriceBufferWritePosition = (uint32_t*)p; }while(0); }while(0); } __set_PRIMASK(primaskstate); };
        }
        v = LL_USART_ReceiveData8(((USART_TypeDef *) (0x40000000UL + 0x00004400UL)));
        rxBuf[index] = (char)v;
        index += index < 120 ? 1 : 0;
        if( v == 0 ){
            do { uint32_t ssiz = strlen( rxBuf ); do { uint32_t limit = 120 -8; uint32_t len_ = ssiz; if( len_ > limit ){ { uint32_t primaskstate = __get_PRIMASK(); __disable_irq(); { do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (0x4000|(2240)); TriceBufferWritePosition = (uint32_t*)p; }while(0);; do{ uint16_t v = ((8)<<8) | TriceCycle++; do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (v); TriceBufferWritePosition = (uint32_t*)p; }while(0); }while(0); do{ *TriceBufferWritePosition++ = ((uint32_t)(len_)); }while(0); do{ *TriceBufferWritePosition++ = ((uint32_t)(limit)); }while(0); } __set_PRIMASK(primaskstate); }; len_ = limit; } { uint32_t primaskstate = __get_PRIMASK(); __disable_irq(); { { uint32_t ts = TriceStamp32(); do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = ((0xC000|(1202))); TriceBufferWritePosition = (uint32_t*)p; }while(0); do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (ts); TriceBufferWritePosition = (uint32_t*)p; }while(0); do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (((ts) >> 16)); TriceBufferWritePosition = (uint32_t*)p; }while(0);; }; if( len_ <= 127 ){ do{ uint16_t v = ((len_)<<8) | TriceCycle++; do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (v); TriceBufferWritePosition = (uint32_t*)p; }while(0); }while(0); }else{ do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = ((0x8000 | (len_))); TriceBufferWritePosition = (uint32_t*)p; }while(0); TriceCycle++; } do{ memcpy( TriceBufferWritePosition, rxBuf, len_ ); TriceBufferWritePosition += (len_+3)>>2; }while(0); } __set_PRIMASK(primaskstate); } } while(0); } while(0);
    ...
    if( LL_USART_IsActiveFlag_TXE(((USART_TypeDef *) (0x40000000UL + 0x00004400UL))) ){
