# 1 "../../../src/triceDoubleBuffer.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 365 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../../../src/triceDoubleBuffer.c" 2
...
static uint32_t triceBuffer[2][(((0x1000/2) + 3) & ~3)>>2] = {0};
static int triceSwap = 0;
    uint32_t* TriceBufferWritePosition = &triceBuffer[0][100>>2];
static uint32_t* triceBufferWriteLimit = &triceBuffer[1][100>>2];



static uint32_t* triceBufferSwap( void ){
    { uint32_t primaskstate = __get_PRIMASK(); __disable_irq(); {
    triceBufferWriteLimit = TriceBufferWritePosition;
    triceSwap = !triceSwap;
    TriceBufferWritePosition = &triceBuffer[triceSwap][100>>2];
...
void TriceLogBufferInfo( void ){
    { uint32_t primaskstate = __get_PRIMASK(); __disable_irq(); { do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (0x4000|(2572)); TriceBufferWritePosition = (uint32_t*)p; }while(0);; do{ uint16_t v = ((4)<<8) | TriceCycle++; do{ uint16_t* p = (uint16_t*)TriceBufferWritePosition; *p++ = (v); TriceBufferWritePosition = (uint32_t*)p; }while(0); }while(0); do{ *TriceBufferWritePosition++ = ((uint32_t)((((0x1000/2) + 3) & ~3))); }while(0); } __set_PRIMASK(primaskstate); };
}
