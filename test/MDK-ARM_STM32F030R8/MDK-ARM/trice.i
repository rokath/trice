...
# 7 "../../../src/trice.c" 2
# 16 "../../../src/trice.c"
static uint32_t triceCommands[(120 +3)>>2];


char* const triceCommandBuffer = (char*)&triceCommands;


int triceCommandLength = 0;


int triceCommandFlag = 0;
...
static int nextTrice( uint8_t** buf, size_t* pSize, uint8_t** pStart, size_t* pLen ){
    ...
    *pStart = *buf;
    switch( triceType ){
        default:
        case 1:
            len = 4 + triceDataLen(*pStart + 2);
            break;
        case 2:
            *pStart += 2;
            offset = 2;
            len = 6 + triceDataLen(*pStart + 4);
            break;
        case 3:
            len = 8 + triceDataLen(*pStart + 6);
            break;
        case 0:
            return -99;
    }
    ...
    if( !( triceSize - (offset + 3) <= len && len <= triceSize - offset )){
        triceErrorCount++;
        return -110;
    }
    size -= triceSize;
...
void TriceWriteDevice( TriceWriteDevice_t device, uint8_t *buf, size_t len ){
    switch( device ){
        ...
        case UartA:
                ...
                triceNonBlockingWriteUartA( buf, len );
...
void TriceOut( uint32_t* tb, size_t tLen ){
    ...
    if( (1 < triceID) && (triceID < ((1<<14)-1)) ){
        TriceWriteDevice( UartA, enc, encLen );
    }
# 229 "../../../src/trice.c"
}
# 246 "../../../src/trice.c"
...
void triceTriggerTransmitUartA(void){
    if( TriceOutDepthUartA() && triceTxDataRegisterEmptyUartA() ){
        triceEnableTxEmptyInterruptUartA();
    }
}
# 346 "../../../src/trice.c"
unsigned TriceOutDepth( void ){
