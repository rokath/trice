...
#if STM32F4XX_CMSIS_CLKSRC == STM32F4XX_CMSIS_CLKSRC_HSI
#endif
#if STM32F4XX_CMSIS_CLKSRC == STM32F4XX_CMSIS_CLKSRC_HSE
#  define SYSCLOCK_SETUP_HSE
#  define SYSCLOCK_CLKSRC  RCC_CFGR_SW_HSE
#endif
#if STM32F4XX_CMSIS_CLKSRC == STM32F4XX_CMSIS_CLKSRC_PLLCLKBYHSI
#  define SYSCLOCK_SETUP_PLL
#  define SYSCLOCK_PLL_INPUT  RCC_PLLCFGR_PLLSRC_HSI
#  define SYSCLOCK_CLKSRC  RCC_CFGR_SW_PLL
#endif
#if STM32F4XX_CMSIS_CLKSRC == STM32F4XX_CMSIS_CLKSRC_PLLCLKBYHSE
#  define SYSCLOCK_SETUP_HSE
#  define SYSCLOCK_SETUP_PLL
...
#define SYSCLOCK_FLASHLATENCY (STM32F4XX_CMSIS_CLOCKHZ / 30000000)
...
uint32_t SystemCoreClock = STM32F4XX_CMSIS_CLOCKHZ;
...
static void SetSysClock(void)
{
  ...
  RCC->CFGR |= (STM32F4XX_CMSIS_MCO1SRC << 21) | (STM32F4XX_CMSIS_MCO1PRE << 24);
  RCC->CFGR |= (STM32F4XX_CMSIS_MCO2SRC << 30) | (STM32F4XX_CMSIS_MCO2PRE << 27);

  /* At this stage the HSI is enabled and used as System clock source */

#ifdef SYSCLOCK_SETUP_HSE
// use pincfg document setting or backup from core plugin
# if defined(PINCFG_RCC_OSC_HSE_EXTERNAL_CLOCK_SOURCE) || (defined(STM32F4XX_CMSIS_HSE_BYPASS) && STM32F4XX_CMSIS_HSE_BYPASS)
...
# endif
  ...
  {
    /* HCLK */
    RCC->CFGR |= (STM32F4XX_CMSIS_AHB << 4);

    /* PCLK2 */
    RCC->CFGR |= (STM32F4XX_CMSIS_APB2 << 13);

    /* PCLK1 */
    RCC->CFGR |= (STM32F4XX_CMSIS_APB1 << 10);

#ifdef SYSCLOCK_SETUP_PLL
    /* Configure the main PLL */
    RCC->PLLCFGR = STM32F4XX_CMSIS_PLL_M | (STM32F4XX_CMSIS_PLL_N << 6) | (STM32F4XX_CMSIS_PLL_P << 16) |
                   (SYSCLOCK_PLL_INPUT) | (STM32F4XX_CMSIS_PLL_Q << 24);
